...
def solve_puzzle(input: str):
    ...
    sum_of_game_power = 0
    for _, game_combinations in games.items():
        max_cubes = {'red':0, 'green':0, 'blue':0}
        for _, cube_combinations in game_combinations.items():
            cube_combinations = dict({'red':0, 'green':0, 'blue':0}, **cube_combinations)
            max_cubes['red'] = max(max_cubes['red'], cube_combinations['red'])
            max_cubes['green'] = max(max_cubes['green'], cube_combinations['green'])
            max_cubes['blue'] = max(max_cubes['blue'], cube_combinations['blue'])
        sum_of_game_power += max_cubes['red'] * max_cubes['green'] * max_cubes['blue']
