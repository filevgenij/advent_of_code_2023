...
def solve_puzzle(input: str):
    ...
    sum_of_lenght = 0
    for pair in pairs:
        fr, fc = [int(x) for x in pair[0].split('_')]
        lr, lc = [int(x) for x in pair[1].split('_')]
        r_numbs = [fr, lr]
        c_numbs = [fc, lc]
        r_numbs.sort()
        c_numbs.sort()
        empty_line = 0
        for r in empty_row:
            if r_numbs[0] < r and r_numbs[1] > r:
                empty_line +=1
        for c in empty_col:
            if c_numbs[0] < c and c_numbs[1] > c:
                empty_line +=1
        
        sum_of_lenght += len(get_closer_distance(*pair, len(image), len(image[0]))) + (empty_line * (1000000 - 1))
